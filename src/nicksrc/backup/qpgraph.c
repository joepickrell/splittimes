...
// differences from basepoint 
  ...
  scale = dofstnumx(fst, f2, f2sig, xsnplist, xindex, xtypes, nrows, ncols, numeg, nblocks, indivmarkers, dmode) ;     
...
void printmatwf(double *a, int m, int n, int w, char *format)
/**
 print a matrix n wide m rows  w to a rowwith format
*/
{
      int i,j, jmod ;
      if (format == NULL) { 
...

{
  ...
  while ((i = getopt (argc, argv, "p:g:s:o:l:vV")) != -1) {
